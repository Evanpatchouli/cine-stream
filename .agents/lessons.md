...
- 后端新增分页接口时必须复用统一结构 `{ list, page, size, total, totalPages }`；如果提供分页 helper，也要同步包含 `totalPages`，避免同仓库不同分页响应结构分裂。
- 全局状态如果承担跨页面语义（例如收藏 store 用于播放页判断是否已收藏），页面筛选结果不要直接覆盖它；筛选列表应使用页面局部状态或独立查询缓存，避免把“全量状态”污染成“过滤后的局部状态”。
- 用户播放偏好一旦有跨设备需求，应以后端用户字段为准；播放页可以保留 localStorage 作为接口失败时的兜底，但设置页必须写后端。
- 客户端公开列表接口没有查询参数时，不要为了接 UI 搜索而伪造无效 query；先做本地过滤并把后端搜索接口作为后续增强点记录下来。
